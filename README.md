...
| `SMTP_*` / `EMAIL_FROM` | Optional digest email |
| `CRON_SECRET` | Optional guard for digest tick endpoint |
...
- **Today** — date navigation, week fit, hybrid match (rules + optional AI polish), feedback, allergy-aware reasons  
- **Menu** — shared board for the selected day (includes always-on Salad Compose)  
- **Preferences** — diet, avoids, likes, freeform notes  
- **Settings** — digest schedule, account, sign out  
